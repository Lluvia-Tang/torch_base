...
if __name__ == "__main__":
    ...
    spacy_nlp = spacy.load("en_core_web_sm") # 'en_core_web_sm'
    spacy_nlp.add_pipe("replace_word", last=True)

    config = AutoConfig.from_pretrained('roberta-base', cache_dir=model_args.cache_dir)
    tokenizer = AutoTokenizer.from_pretrained('roberta-base', cache_dir=model_args.cache_dir, config=config)
    ...
    txt = []
    with open("/workspace/torch_base/data/clean_corpus/test_clean.txt", 'r') as f:
        while True:
            fa = f.readline()
            if fa == "":
                break
            txt.append(fa)
    syn_txt = open("test_syn_clean.txt",'w')
    ant_txt = open("test_ant_clean.txt",'w')
    ...
    # txt = ["are you kidding me i am the daughter and granddaughter of holocaust survivors and comparing wearing a mask to nazis is beyond the pale of offensive by the way i am also a utahn and i am more than happy to be considerate so wearadamnmask",
    #        "Are you kidding me? I am the daughter and granddaughter of Holocaust survivors, and comparing wearing a mask to Nazis is beyond the pale of offensive. By the way, I am also a Utahn and I am more than happy to be considerate, so #WearADamnMask"]
    docs = spacy_nlp.pipe(txt, n_process=1, batch_size=100, disable=['parser', 'ner'])
    for doc in docs:
        print(" ".join([t.text for t in doc])) #原句子
        print(" ".join(doc._._synonym_sent)) #近义句
        # syn_txt.append(" ".join(doc._._synonym_sent))
        syn_txt.write(" ".join(doc._._synonym_sent))
        print(" ".join(doc._._antonym_sent)) #反义句
        ant_txt.write(" ".join(doc._._antonym_sent))
        # ant_txt.append(" ".join(doc._._antonym_sent))

    syn_txt.close()
    ant_txt.close()

'''
python utils/generate_syn_ant.py \
    --cache_dir ./cache \
