...
from model.GCN.lstm import DynamicLSTM
...
class MYGCN(nn.Module):
    def __init__(self, hidden_size, embedding_dim, embedding_matrix):
        super(MYGCN, self).__init__()
        self.hidden_dim = hidden_size
        self.embedding_dim = embedding_dim
        self.embed = nn.Embedding.from_pretrained(torch.tensor(embedding_matrix, dtype=torch.float))
        self.text_lstm = DynamicLSTM(embedding_dim, hidden_size, num_layers=1, batch_first=True, bidirectional=True)
        self.gcn1 = GraphConvolution(2*hidden_size, 2*hidden_size)
        self.gcn2 = GraphConvolution(2*hidden_size, 2*hidden_size)
        self.gcn3 = GraphConvolution(2*hidden_size, 2*hidden_size)
        self.gcn4 = GraphConvolution(2*hidden_size, 2*hidden_size)
        self.gcn5 = GraphConvolution(2*hidden_size, 2*hidden_size)
        self.gcn6 = GraphConvolution(2*hidden_size, 2*hidden_size)
        self.gcn7 = GraphConvolution(2*hidden_size, 2*hidden_size)
        self.gcn8 = GraphConvolution(2*hidden_size, 2*hidden_size)

        self.fc = nn.Linear(2*hidden_size, 3)
        self.dfc = nn.Linear(4*hidden_size, 3)
        ...
        self.sentiment = nn.Linear(2*hidden_size, 3)
    ...
    def forward(self, inputs):
        text_indices, target_indices, adj = inputs[0], inputs[2], inputs[5]
        ...
        # print(f'text_out size : {text_out.size()}')
        # print(f'adj size {adj.size()}')

        x = F.relu(self.gcn1(text_out, adj))
...
if __name__=="__main__":
    ...
    config = BertConfig.from_pretrained("/root/files/bert-base-uncased")
    config.gcn_layers = 3
    config.polarities_size = 3
    model = ICGCNBert.from_pretrained("/root/files/bert-base-uncased", config=config)
